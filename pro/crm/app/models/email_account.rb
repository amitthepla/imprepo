class EmailAccount
  ...
  ACCOUNT_PROVIDERS = %W(GMAIL YMAIL OUTLOOK).freeze
  ...
  def refresh_access_token!
    if self.provider == 'outlook'
      token = OAuth2::AccessToken.from_hash(OUTLOOK_CLIENT, self.attributes.dup)
      new_token = token.refresh!
      self.update_attributes({
                                 access_token: new_token.token,
                                 refresh_token: new_token.refresh_token,
                                 expires: new_token.expires_at.present?,
