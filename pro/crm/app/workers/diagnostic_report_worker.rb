class DiagnosticReportWorker
  ...
  def perform(org_id, user_id, date_hash)
    begin
      @current_org = Business::Organization.find(org_id)
      @user = @current_org.users.find(user_id)

      first_start_date = Date.strptime(date_hash["first_start"], '%m/%d/%Y')
      first_end_date = Date.strptime(date_hash["first_end"], '%m/%d/%Y')
      second_start_date = Date.strptime(date_hash["second_start"], '%m/%d/%Y')
      second_end_date = Date.strptime(date_hash["second_end"], '%m/%d/%Y')

      first_activity_data(first_start_date, first_end_date)
      second_activity_data(second_start_date, second_end_date)


      report = ApplicationController.new.render_to_string('/marketing/reports/diagnostic.html.erb', :layout => 'report',
                  :locals => {
          ...
          Business::DiagnosticReport.create!(
