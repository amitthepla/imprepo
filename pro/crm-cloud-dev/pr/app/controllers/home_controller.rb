...
#*****************************************************************************************/
  ...
  skip_before_filter  :authenticate_user!, :only => [:index, :pricing, :notfound, :terms, :privacy, :security,:contact_us, :clear_cache,:api_contact_us, :download_user, :features,:integration, :faq, :awards_and_recognitions, :about_us, :roadmap,:report_a_bug, :releases, :save_bug_report, :pricing, :sitemap, :lambda_test, :org_trial_form]  
  caches_action :terms,:privacy#, :security
  
  
  def index   
    @title = "WakeUpSales | Free CRM Tool | Client Management Software​"
    @description = "WakeUpSales is the best free CRM tool to streamline the sales activities, manage leads, create powerful customer relationships and improve the productivity with CRM Solution."
    get_keywords
  ...
  def dashboard
    if @current_user.is_siteadmin?
      ...
      begin
        ...
        @tasks=@current_organization.tasks.where("is_completed=? AND DATE_FORMAT(DATE_ADD(due_date, INTERVAL #{Time.zone.now.utc_offset} second), '%Y/%m/%d')=? ", false, Time.zone.now.strftime("%Y/%m/%d"))
        ...
        @qualified_deals = qualified_ds.present? ? @current_organization.deals.where(:is_active=>true,:deal_status_id=>@current_organization.filter_deal_status(qualified_ds.id,@current_organization.id).id).count : 0
        @won_deals = won_ds.present? ? @current_organization.deals.where(:is_active=>true,:deal_status_id=>@current_organization.filter_deal_status(won_ds.id,@current_organization.id).id).count : 0
        @lost_deals = lost_ds.present? ? @current_organization.deals.where(:is_active=>true,:deal_status_id=>@current_organization.filter_deal_status(lost_ds.id,@current_organization.id).id).count : 0

        @task_call_count = Task.last_three_months.task_list(current_user,nil,nil,nil,'Call').count
        @total_leads = @current_organization.deals.where(is_active: true).count
        ...
        begin 
          ...
          if (@current_user.is_admin? || @current_user.is_super_admin?)
            last_deal_condition.where("deals.organization_id =? AND deal_statuses.original_id IN (?) AND deals.is_active=?", @current_user.organization_id, [6], true)
          else
            last_deal_condition.where(" deals.organization_id =? AND deal_statuses.original_id IN (?) AND deals.is_active=? AND (deals.assigned_to=? or initiated_by=?)", @current_user.organization_id, [6], true, @current_user.id, @current_user.id)
          end
          ...
          if @last_deal.present?
             ...
             @last_closed_user = deal_move.user.full_name.present? ? deal_move.user.full_name : deal_move.user.email
          end
        rescue 
          @last_deal = nil
      ...
      #   @users = User.order("updated_at desc")
      # end
      @title = "WakeUpSales | Free CRM Tool | User Dashboard"
      @description = "WakeUpSales free CRM tool user dashboard is the place where user can check all his activities relating to sales and CRM solutions."
    end
  end
  def organizations
    if @current_user.is_siteadmin?
  ...
  def contact_us
    @title = "Contact WakeUpSales | Free CRM Tool | Lead Management Tool"
    @description = "Contact WakeUpSales for free CRM Tool and lead management and customer management software. We will all types of support in installation and maintenance."
    get_keywords
    if request.post?
  ...
  def activities
    if @current_user.is_admin?
      ...
      @title = "WakeUpSales | Free CRM Tool |User Activity"
      @description = "WakeUpSales the free crm tool registered users can manage their activity easily."
      respond_to do |format|
        format.html
...
group by created_by order by count(*) desc")
 ...
 def usage
  ...
  def features
    @title = "WakeUpSales | Free CRM Tool | Best CRM Application Features"
    @description = "WakeUpSales is the best free CRM tool to streamline sales activities, manage leads, create powerful customer relationships and improve the productivity with lots of innovative features.​"
    get_keywords
  end
  ...
  def getting_started
    ...
    @title = "Getting Started With WakeUpSales | Free CRM Tool"
    @description = "Get started with WakeUpSales free crm tool to manage your leads, clients, sales and other activities to improve the productivity."
  end
 ...
 def download_user
  ...
  def integration
    @title = "WakeUpSales | Free CRM Tool | Best CRM Software Integration"
    @description = "WakeUpSales is the best free CRM tool to streamline the sales activities, manage leads and other. Third party apps like zendesk, Help Scout, zapier and email tracker can easily integrate to this.​"
    get_keywords
  end
  def terms
    @title = "WakeUpSales | Free CRM Tool | Terms and Conditions"
    @description = "WakeUpSales the free crm tool general terms and conditions for user’s acknowledgement before using this tool.​"
    get_keywords
  end
  def privacy
    @title = "WakeUpSales | Lead Management and Free CRM Tool | Privacy Policy"
    @description = "WakeUpSales the client management and lead management software applies Privacy Policy to all of its products and services.​"
    get_keywords
  end
  def security
    @title = "WakeUpSales | Best Free CRM Tool | Security​"
    @description = "WakeUpSales the free CRM tool conducts routine security audits by top notch experts in the industry. We use physical and technical safeguards to preserve the integrity and security of your information.​"
    get_keywords
  end
  def faq
  ...
  def awards_and_recognitions
    @title = "WakeUpSales | Free CRM Tool | Awards and Recognitions"
    @description = " WakeUpSales is a great place to work and interact with customers and employees around the world. Awards and recognitions we received from various top source to do better, every day"
    get_keywords
  end
  def about_us
    @title = "About WakeUpSales | Free CRM Tool | Lead and Client Management Software​"
    @description = "WakeUpSales is the best free CRM tool to streamline the sales activities, manage leads, create powerful customer relationships. We have customized add-ons and Plugins to make your work easier. "
    get_keywords
  end
  def roadmap
    @title = "WakeUpSales | Free CRM Tool | Free CRM Software Roadmap​"
    @description = "WakeUpSales is the best free CRM tool to streamline the sales activities, manage leads, create powerful customer relationships. Check the roadmap of free CRM software."
    get_keywords
  end
  def releases
    @title = "WakeUpSales | Free CRM Tool | CRM Software Updates"
    @description = "Here you will check all the new updates of WakeUpSales the best free CRM tool to streamline your sales activities, manage leads and other."
    get_keywords
  end
  def report_a_bug
    @title = "WakeUpSales | Free CRM Tool | Lead Management Software Bug Report​"
    @description = "WakeUpSales is the best free CRM tool to streamline the sales activities, manage leads. We are always trying to make our product bug free by fixing your bugs you find in it.​"
    get_keywords
  ...
  # Save Bug Report and send email to User and support@wakeupsales.com
  def save_bug_report
    if request.xhr?  
    else  
      begin
        ...
        if bug_report = BugReport.create(params[:bug_report])
          Notification.send_bug_report_to_user(params[:bug_report][:name],params[:bug_report][:email]).deliver
  ...
  def pricing
    @title = "WakeUpSales | Free CRM Tool |Best CRM Application Pricing"
    @description = "WakeUpSales is a great place to work and interact with customers and employees around the world. We have customized Plugins to make your work simple. Check our pricing details."
    get_keywords
  end
  def sitemap
  ...
  def org_trial_form
    ...
    render :partial => "update_org_trial"
  end

  def update_org_trial
    @organization = Organization.where("id=?",params[:org_id]).first
    @organization.update_attributes(:user_allowed_by_admin => params[:user_allowed_by_admin], :extend_trial_days => params[:extend_trial_days])
    render :text => "success"
  end
end
