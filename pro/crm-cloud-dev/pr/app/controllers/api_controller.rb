...
def create_download_lead
  ...
  contactid= save_company_contact(email_splt[0],params[:email],"","",admin_user.id,"","","",@org)
  begin
  ...
  if contactid     
    ...
    if params[:reference].nil? || params[:reference].blank?
      title= "New Download by #{params[:email]} from wakeupsales.org"
    elsif params[:reference] == "andolasoft"
      title= "#{params[:name]} contacted from andolasoft.com with email - #{params[:email]}"
    end
    deal.title=title
    ...
    if web_form.present?
      ...
      if web_form.send_email_notification
        ...
        if web_form.email_to == "all"
          cc_user_email = org.users.where("id=?",web_form.user_responsible).first.email
          if user_email == cc_user_email
            cc_user_email = ""
          end
        end
        # Send notification to Super admin and assigned user
        Notification.send_create_form_notification(user_email,cc_user_email,web_form.form_name, params[:email]).deliver
        # Send thank you email to the lead
      end
      Notification.send_web_form_thank_you_to_user(params[:first_name]+" "+params[:last_name], params[:email], org.name).deliver

      # Redirect to page according to selection in web form
      if web_form.is_display_thank_you_page
        redirect_url = "http://www.wakeupsales.com/web_form/thank_you"
      else
