    ...
    CSV.generate do |csv|
      ...
      all.each do |task|
        ...
        csv << [task.title, 
                ...
                deal.created_at,
                deal.initiator_name,
                deal.priority_type.present? ? deal.priority_type.name : "NA",
                deal.is_public? ? "Public" : "Everyone",
...
def insert_in_google_calendar(user)
  puts "-------------- insert_in_google_calendar"
  p user
...
#    formated_event_end_date= self.event_end_date.in_time_zone(user.time_zone)
  ...
  def get_url
    if self.taskable.present?
      self.taskable.class.name == "Deal" ? "/leads/"+ self.taskable.to_param.to_s : (self.taskable.class.name == "CompanyContact" ? "/company_contact/"+ self.taskable.to_param.to_s : "/individual_contact/"+ self.taskable.to_param.to_s)
    end
  end
  ...
  def assigned_user_name
    user.present? && user.full_name.present? ? user.full_name : (user.email.present? ? user.email.split("@")[0] : "Not Available")
  end
