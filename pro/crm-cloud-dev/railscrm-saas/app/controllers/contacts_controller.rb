...
#      if params[:selected_tab] == "company"
  ...
  def send_email
    ...
    activity = Activity.create(:organization_id => current_user.organization.id, :activity_user_id => current_user.id, :activity_type => mail_letter.class.name, :activity_id => mail_letter.id, :activity_status => "Mail Sent", :activity_desc => mail_letter.subject, :activity_date => mail_letter.updated_at, :is_public => (mail_letter.mailable.nil? || mail_letter.mailable.is_public.nil? || mail_letter.mailable.is_public.blank?) ? false : mail_letter.mailable.is_public, :source_id => mail_letter.mailable.present? ? mail_letter.mailable.id : "")

    # Save email info before sending to the lead.
    save_sent_email
    # Send email to lead
    Notification.send_email(params[:to], params[:cc], params[:bcc], params[:subject], params[:message], file_name, file, activity.id, full_name, params[:mailer_id],current_user.email).deliver
  ...
  def contact_widget
    if params[:tab_type].present? && params[:id].present? && params[:contact_type].present?
      params[:type]=params[:contact_type].present? && params[:contact_type] == "CompanyContact" ? "company" : "individual"
      show_contact_detail
