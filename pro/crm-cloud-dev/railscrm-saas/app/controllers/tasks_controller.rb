...
#    @tasks=Task.task_list(current_user, "all")
    ...
    if @task.present? && @user.present?
      @task.insert_in_google_calendar(@user)
    ...
    @title = "Task Listing | Task Management Software | WakeUpSales"
    @description = "WakeUpSales task management tool, user task listing page. User can view, assign and customize all his task in this page."
...
#          task.created_by = current_user.id
            if save_task(params, taskable)
              ...
              if params[:task][:notify_email].to_i == 1
    ...
    puts "#######################################"
     ...
     if params[:task_out_val].present?
      params[:task_out_val].chop.split(',').each do |f|
        ...
        p "params[:task_out_val] : #{params[:task_out_val]}"
        p "---------task_ot_cm : #{task_ot_cm.id}"
...
#   @tasks=@tasks.includes(:task_type).where("is_completed=? AND task_types.id IN (?)", false, params[:ids])
  ...
  def destroy
    ...
    t_type = deal.last_task.present? ? deal.last_task.name : "No-Action"
    respond_to do |format|
  ...
  def calendar_data
    if request.xhr?
      ...
      task.due_date= task.created_at unless task.due_date.present?
			...
			events << {:id => task.id, :img=>imgurl, :is_complete=> task.is_completed, :tasktype=> task.task_type.present? ? task.task_type.name : "None" , :assign_to=> "#{(task.user.present? ? task.user.full_name : 'NA')}", :url=> "http://#{request.host_with_port}/leads/#{deal.to_param}", :title => "\n"+task.title+ "\n" + "Deal:" +" "+deal.title+ "\n" ,   :color=> TaskType::TASK_COLORS[task.task_type.present? ? task.task_type.name : "None"], :className=>"test", :description => "At - #{task.due_date.strftime('%H:%M')}\nTask - "+task.title+"\n Deal - " + deal.title+"\nAssigned To - "+(task.user ? task.user.full_name : ""), :start => "#{task.due_date.iso8601}", :end => "#{task.due_date.iso8601}", :allDay => false}
	    end
      end
    end
    respond_to do |format|
