...
#*****************************************************************************************/
  ...
  def index
    ...
    @title = "Lead Listings | Lead Management Tool | WakeUpSales"
    @description = "WakeUpSales lead management tool , lead listing page for user. User can check all his lead in this page."
...
#        contact = Contact.find params[:deal][:contact_id]
    ...
    if contact.present?
      ...
      flash[:bosuccess] = "Yo ho! A new lead has been added. Let's keep going!"
  ...
  def update
    ...
    flash[:bosuccess] = "Your deal info has been updated. It's a good habit to be up-to-date. Yeah!"
    if !request.xhr?
      ...
      flash[:bosuccess] = "Wow! That was quick. But I noticed... Deal info updated!"
      render :text => "success"
    end
  end
  ...
  def move_lead
    ...
    if params[:mass_deal_ids].present?
      p "params[:mass_deal_ids] : #{params[:mass_deal_ids]}"
      response_msg = ""
    ...
    if params[:mass_deal_ids].present?
      ...
      p "~~~~~~~~~~~~~`"
      p deal.assigned_user
      p "deal.assigned_to ======== #{deal.assigned_to}"
      p "``-------------------------------------`"
      p deal
      p "````-----------```"
      p deal.deal_status_name
      p "~~~~~~"
      stage_info={status: true, msg: "Deal has been moved successfully.", stag_name: deal.deal_status_name, assigned_user: deal.assigned_user.present? && deal.assigned_user == @current_user ? "Me" : (deal.assigned_user.present? && deal.assigned_user.full_name.present? ? deal.assigned_user.full_name : "Me"), assigned_user_id: deal.assigned_user.present? ? deal.assigned_user.id : @current_user.id, deal_org_id: deal.deal_status.original_id, total_deal: dealscount}
    end
  ...
  def delete_selected_deals
    ...
    deal_ids.each do |id|
      ...
      flash[:bosuccess] = "Deal(s) has been deleted successfully!"
    end
    respond_to do |format|
  ...
  def destroy
    ...
    flash[:bosuccess] = "Awwww! deal have been deleted. Don't you worry, let's focus on the better ones!"
    respond_to do |format|
  ...
  def accept_assign_deal
    ...
    begin
      ...
      if (deal = Deal.where("hot_lead_token =?", decrypted_lead_token).first).present?
        ...
        flash[:bosuccess] = "Deal has been accepted successfully."
      else
  ...
  def deny_assign_deal
    ...
    begin
      ...
      if (deal = Deal.where("hot_lead_token =?", decrypted_lead_token).first).present?
        ...
        flash[:bosuccess] = "You have successfully denied the request of accepting the deal."
      else
        flash[:bodanger] = "Token has already been expired!"
      end
    rescue => e
  ...
  def change_lead_status
    ...
    dm = DealMove.create(:organization => @current_organization, :deal_id => lead.id, :deal_status_id => status.id, :user => @current_user)
    lead.deal_status_id = status.id
    lead.move_deal = true
    lead.save
