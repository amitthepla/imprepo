...
private
  ...
  def fetch_contacts
    ...
    if cuser.is_super_admin?      
      ...
      individual_contacts = cuser.organization.individual_contacts.where("created_by=?", cuser.id).order("#{sort_column} #{sort_direction}")
      #company_contacts = cuser.organization.individual_contacts.where("created_by=?", cuser.id).order("#{sort_company_column} #{sort_direction}")
    end
